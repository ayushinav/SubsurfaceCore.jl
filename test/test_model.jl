...
function SubsurfaceCore.forward(m::testModel, vars, response_trans_utils = no_tf, params = (;))
    y1 = @. m.x1 + m.x2
    y2 = @. m.x1 * m.x2
    return testResponse(y1, y2)
end

from_nt(testModel, (; x1 = [2.], x2 =[4.]))

function SubsurfaceCore.forward_helper(m::Type{T}, m0, vars, response_trans_utils,
        params) where {T <: testModel}
    model = from_nt(m, m0)
...
mutable struct testModelDistribution{T1 <: Union{Distribution, AbstractArray} , T2 <: Union{Distribution, AbstractArray}} <: AbstractModelDistribution
   x1::T1 
   x2::T2 
end

mutable struct testResponseDistribution{T1 <: Union{Function, Nothing}, T2 <: Union{Function, Nothing}} <: AbstractResponseDistribution
    y1::T1
    y2::T2
end
